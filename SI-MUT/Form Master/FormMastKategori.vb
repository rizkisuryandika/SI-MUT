...
Public Class FormMastKategori
    ...
    Private Sub load_data()
        Try
            DGV1.DataSource = Data("Select * from tab_kategori")
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
    End Sub
    ...
    Private Sub get_nomor()
        ...
        Try
            nomor = ekSkalar("select right(max(id_kategori),3) from tab_kategori")
        Catch ex As Exception
            nomor = 0
        End Try
        If nomor = 0 Then
            tx_kode.Text = "K001"
        Else
            tx_kode.Text = "K" & Format(nomor + 1, "000")
        End If
    End Sub
    ...
    Private Sub ButtonSimpan_Click(sender As Object, e As EventArgs) Handles ButtonSimpan.Click
        Try
            ...
            If ButtonSimpan.Text = "SIMPAN" Then
                sqlku = "insert into tab_kategori (ID_KATEGORI,NAMA,STATUS)
                                         values
                                            ('" & tx_kode.Text & "',
                                            '" & tx_jenis.Text & "',
                                            '" & sts() & "')"
            Else
                sqlku = "UPDATE tab_kategori SET
                             NAMA='" & tx_jenis.Text & "',
                             STATUS='" & sts() & "'
                             WHERE ID_KATEGORI = '" & tx_kode.Text & "'"
            End If
    ...
    Private Sub ButtonHapus_Click(sender As Object, e As EventArgs) Handles ButtonHapus.Click
        Try
            Dim sqlku As String = "DELETE FROM tab_kategori WHERE ID_KATEGORI = '" & tx_kode.Text & "'"
            RUN_Transaction(sqlku)
    ...
    Private Sub get_data()
        Try
            ...
            dt = Data("Select * from tab_kategori where ID_KATEGORI = '" & tx_kode.Text & "'")
    ...
    Private Sub DGV1_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles DGV1.CellDoubleClick
        Try
            Me.tx_kode.Text = DGV1.Rows(i).Cells("ID_KATEGORI").Value
            get_data()
